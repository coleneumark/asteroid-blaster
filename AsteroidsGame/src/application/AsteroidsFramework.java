...
//			boolean died = false;
//			
//			int asteroidCount = 20; // e.g start with about 20 asteroids on screen (need to account for this later per object)
...
//				double delta = (nano - old) / 1e9; // NB, time receiving is in nanoseconds (timestamp) so need to convert it to seconds
//				
//				old = nano; 
//				elapsed += delta;
...
//						));
//						loseLabel.setVisible(true); // now this label will displayed if the player lost/died
//						died = true;
//					}
//					// if asteroid leaves the bounds, destroy it
...
//				
//				for (Bullet bullet : bullets) { 
//					// if the bullet is leaving the bounds, destroy it
...
//			
//		};
//		loop.start(); // start our game loop, will now display the frame rate (60 fps) and time elapsed in seconds on screen, will change with further design
//		
//		
//		stage.show();
//	}
