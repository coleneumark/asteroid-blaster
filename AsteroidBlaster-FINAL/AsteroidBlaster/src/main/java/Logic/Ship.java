...
public class Ship implements IGameObject, IPlayer{
    ...
    public void onCreate() {
        ...
        this.x = Math.round(settings.BOARD_WIDTH/2);
        this.y = Math.round(settings.BOARD_HEIGHT/2);
        this.polygon.setTranslateY(this.y);
        this.polygon.setTranslateX(this.x);
        this.turn(270);
        this.lives = GameSettings.settings.playerStartingLives;
        this.jump = GameSettings.settings.jumpLmt;
        ...
        if(this.engine.getGameState() == GameState.PLAYING){
                switch (code) {
                    case UP:
                        ...
                        Boolean upKeyHeld = pressedKeys.get(KeyCode.UP);
                        if (upKeyHeld != null && upKeyHeld) {
                    ...
                    case DOWN:
                        pressedKeys.put(code, true);
                        Boolean downKeyHeld = pressedKeys.get(KeyCode.DOWN);
                        if (downKeyHeld != null && downKeyHeld) {
                            Point2D speedPosition = calculateNewPosition();
                            if (getSpeed() >= 0) {
                                this.speed -= .2;
                                anchor = anchor.add(speedPosition);
                    ...
                    case SPACE:
                        pressedKeys.put(code, true);
                        Boolean spaceKeyHeld = pressedKeys.get(KeyCode.SPACE);
                        if (spaceKeyHeld != null && spaceKeyHeld) {
                            this.shoot();
                        }
                        break;
                }
    ...
    private void checkIfInbound(){
        if (this.polygon.getBoundsInParent().getCenterX() < 0) {
            this.polygon.setTranslateX(this.polygon.getTranslateX() + this.settings.BOARD_WIDTH);
        }

        if (this.polygon.getBoundsInParent().getCenterX() > this.settings.BOARD_WIDTH) {
            this.polygon.setTranslateX(this.polygon.getTranslateX() % this.settings.BOARD_WIDTH);
        }

        if (this.polygon.getBoundsInParent().getCenterY() < 0) {
            this.polygon.setTranslateY(this.polygon.getTranslateY() + this.settings.BOARD_HEIGHT);
        }

        if (this.polygon.getBoundsInParent().getCenterY() > this.settings.BOARD_HEIGHT) {
            this.polygon.setTranslateY(this.polygon.getTranslateY() % this.settings.BOARD_HEIGHT);
        }
    }
    ...
    private void hyperSpace() {
        ...
        if (jumpAllow) {
            ...
            while (intersects) {
                ...
                this.getPolygon().setTranslateX(rnd.nextInt(this.settings.BOARD_WIDTH));
                this.getPolygon().setTranslateY(rnd.nextInt(this.settings.BOARD_HEIGHT));
