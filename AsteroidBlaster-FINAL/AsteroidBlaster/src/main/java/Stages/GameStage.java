        ...
        Platform.runLater(() -> {
            ...
            for(IGameObject item : this.gameObjects){
                if(item.getIsAlive()){//if the item is still alive add it to the temp list
                    ...
                    // implement 1000 points reached, granted additional life
                    int scoreRounded = (int) Math.floor(this.score / 1000.0) * 1000;
                    if (scoreRounded > this.livesGranted * 1000) {
                        this.livesGranted++;
                        this.ship.addLife();
                    }
                }else if(IAlienShip.class.isAssignableFrom(item.getClass())){
                    if (!item.getIsAlive()){
                        this.score += 250;
                    }
                }
            ...
            int jumpNum = ship.getJump();
            StringBuilder shipLogo = new StringBuilder();
            ...
            Text jump = new Text("JUMPS: " + shipLogo.toString());
            jump.setFill(Color.WHITE);
            jump.setFont(new Font("Arial", 20));
    ...
    public void loadStage() { // called as soon as stage is set
        ...
        Platform.runLater(() -> {
            ...
            this.scene.setOnKeyPressed((event) -> {
                ...
                this.ship.handleKeyPress(event.getCode()); // changes from .getCode()
             });
